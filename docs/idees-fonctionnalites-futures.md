...
Vue colorée : quelles variables manquent, chez quels patients ; clic = aller compléter. **v1 livrée** : section « Complétude par variable » dans l'onglet Statistiques — barre colorée par variable (vert ≥ 80 %, ambre ≥ 50 %, rouge en dessous), les moins complètes d'abord ; champs rencontre rapportés aux types concernés ; code manquant explicite = renseigné. **Renfort audit 2026-07-08** : la vue par défaut est désormais historique, chaque dossier est évalué contre sa propre version de gabarit ; le mode `current` reste disponible pour l'harmonisation volontaire ; les valeurs observées et les codes manquants documentés sont séparés. RPC `base_completeness_stats(p_base_id, p_mode)` (INVOKER, RLS naturelle), migrations `097000` + `097800`. **Pas encore** : la grille par patient + le clic « aller compléter » (rejoint B2, prochaine itération).
...
Liste automatique des patients/rencontres `draft` avec champs requis manquants, triable, assignable à un membre. **v1 livrée** : onglet « À compléter » (membres avec droit de saisie) — dossiers non finalisés avec les **libellés** des champs requis manquants (chips), bouton « Compléter » → ouvre directement le bon formulaire. Chaque entité évaluée contre SA version de gabarit (§7.4) ; champs de rencontre exigés selon le type ; code manquant explicite = renseigné. **Renfort audit 2026-07-08** : les brouillons sans aucun champ requis manquant sortent de la file, et la RPC paginée `base_completion_queue_page` retourne `items/total/limit/offset/hasMore` pour éviter les listes trop lourdes. RPC `base_completion_queue` conservée comme wrapper compatible, migration `097800`. **Pas encore** : l'assignation à un membre (prochaine itération).
...
Inclusions cumulées vs objectif daté (« 150 patients d'ici décembre »). Livré : onglet « Statistiques » de la base — cartes (inclus / objectif / progression), courbe cumulée SVG avec ligne d'objectif pointillée + barre de progression, formulaire d'objectif (propriétaire). Migration `096900` : colonnes `inclusion_target(_date)` sur `base` + RPC `base_inclusion_stats` (SECURITY INVOKER, RLS naturelle — sans accès : série vide). **Renfort audit 2026-07-08** : la courbe utilise `patient.inclusion_date`, initialisée avec la première date de rencontre non supprimée puis maintenue par trigger, au lieu de dépendre de `created_at`. L'onglet accueille aussi B1.
