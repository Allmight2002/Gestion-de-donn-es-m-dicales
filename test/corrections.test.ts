...
describe('audit v10 §4.2 : verdict d inspection reserve au serveur', () => {
  test('un utilisateur ne peut PAS s auto-attribuer inspection_status=scanning/accepted/quarantined', async () => {
    ...
    const aid = (await db.admin.query(
      "insert into public.clinical_attachment(patient_id, storage_path, deidentification_confirmed, inspection_status) values($1,$2,true,'accepted_client') returning id",
      [pid, `${baseId}/p/x.png`],
    )).rows[0].id;
...
describe('audit v12 §6.3 : metadonnees d un fichier accepted immuables', () => {
  test('apres accepted, un utilisateur ne peut plus alterer octets/pointeur/statut ; le serveur oui', async () => {
    ...
    const aid = (await db.admin.query(
      "insert into public.clinical_attachment(patient_id, storage_path, file_hash, file_size, deidentification_confirmed, inspection_status) values($1,$2,'h-ok',100,true,'accepted') returning id",
      [pid, `${baseId}/p/ok.png`],
    )).rows[0].id;
    // Alice (proprietaire, acces identite) NE PEUT PLUS echanger le fichier inspecte...
    await expect(rowsAs(aliceId, 'update public.clinical_attachment set storage_path=$2 where id=$1', [aid, `${baseId}/p/evil.png`])).rejects.toThrow(/immuable/i);
    ...
    expect((await db.admin.query('select storage_path, file_hash from public.clinical_attachment where id=$1', [aid])).rows[0])
      .toMatchObject({ storage_path: `${baseId}/p/ok.png`, file_hash: 'h-ok' });
