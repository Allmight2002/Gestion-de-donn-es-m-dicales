-- =============================================================================
-- 20260711000100_upload_operation_idempotence.sql
-- Durable, server-side identity for an upload operation.  A retry reuses the
-- same ticket/path and returns the already attached business row.
-- =============================================================================

alter table public.upload_ticket
  add column if not exists idempotency_key uuid,
  add column if not exists file_hash text,
  add column if not exists file_size bigint,
  add column if not exists mime_type text;

create unique index if not exists ux_upload_ticket_owner_operation
  on public.upload_ticket(owner_user_id, idempotency_key)
  where idempotency_key is not null;

alter table public.clinical_attachment
  add column if not exists upload_ticket_id uuid references public.upload_ticket(id);
alter table public.raw_document
  add column if not exists upload_ticket_id uuid references public.upload_ticket(id);

create unique index if not exists ux_clinical_attachment_upload_ticket
  on public.clinical_attachment(upload_ticket_id) where upload_ticket_id is not null;
create unique index if not exists ux_raw_document_upload_ticket
  on public.raw_document(upload_ticket_id) where upload_ticket_id is not null;
...
begin
  ...
  if p_bucket = 'clinical-attachments' then
    select id into document_id from public.clinical_attachment where upload_ticket_id = v_ticket.id;
  elsif p_bucket = 'raw-documents' then
    select id into document_id from public.raw_document where upload_ticket_id = v_ticket.id;
  end if;
  return next;
end $$;
...
begin
  ...
  if v_ticket.status = 'attached' then
    if p_entity = 'attachment' then select id into v_id from public.clinical_attachment where upload_ticket_id = v_ticket.id;
    elsif p_entity = 'raw_document' then select id into v_id from public.raw_document where upload_ticket_id = v_ticket.id;
...
-- Historical rows remain unreadable unless they already have a documented server verdict.
-- Operators can use existing quarantine reconciliation; dangling tickets are handled by cleanup-upload.