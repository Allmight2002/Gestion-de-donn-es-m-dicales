-- =============================================================================
-- 20260616097700_atomic_file_inspection.sql
-- Robustesse inspection :
--   - chaque scan porte un inspection_run_id ;
--   - la finalisation du verdict et l'audit sont transactionnels ;
--   - un scan perime ne peut plus ecraser le verdict d'un scan plus recent.
-- =============================================================================
...
create index if not exists ix_clinical_attachment_inspection_run
  on public.clinical_attachment (inspection_run_id)
  where inspection_status = 'scanning';

create index if not exists ix_raw_document_inspection_run
  on public.raw_document (inspection_run_id)
  where inspection_status = 'scanning';
  ...
  if TG_OP = 'UPDATE' and auth.uid() is not null then
    foreach col in array protected loop
      if (old.inspection_status = 'accepted'
          or col in ('inspection_run_id','inspection_started_at'))
