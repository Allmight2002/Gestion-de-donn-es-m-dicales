-- =============================================================================
-- 20260616095100_import_cross_batch_idempotence.sql  (audit v12 §7.8)
-- Les empreintes de lignes (import_row_hash) etaient liees AU LOT : rejouer un fichier corrige
-- (nouveau lot, nouveau hash de fichier) re-creait les rencontres des lignes qui avaient DEJA
-- reussi. On rend l'idempotence INTER-LOTS, par BASE :
--   * import_row_hash porte desormais base_id (backfill depuis le lot, puis NOT NULL) ;
--   * a l'import (apercu inclus), une ligne dont l'empreinte (patient|date|type|md5(donnees))
--     existe deja pour la base via un AUTRE lot est IGNOREE (compteur `already_imported`),
--     ni re-creee ni comptee en erreur -> « reprendre uniquement les lignes en erreur » devient
--     simplement « rejouer le fichier corrige » ;
--   * les imports AUTONOMES (<= 300 lignes) enregistrent desormais eux aussi leurs empreintes
--     (rattachees a leur import_batch de bilan), donc participent a l'idempotence ;
--   * seules les lignes REUSSIES laissent une empreinte (le sous-bloc par ligne annule l'insert
--     d'empreinte en cas d'erreur ; en autonome, on collecte ok_rows apres reussite complete).
-- La dedup INTRA-fichier (§6.3) est inchangee (erreur de ligne).
-- Migration ADDITIVE (colonne + redefinition d'import_records, corps repris de 094500).
-- =============================================================================

-- 1) Empreintes par BASE ------------------------------------------------------
alter table public.import_row_hash
  add column if not exists base_id uuid references public.base(id) on delete cascade;
update public.import_row_hash h
   set base_id = b.base_id
  from public.import_batch b
 where b.id = h.batch_id and h.base_id is null;
alter table public.import_row_hash alter column base_id set not null;
create index if not exists import_row_hash_base_idx on public.import_row_hash (base_id, row_hash);

-- 2) import_records : idempotence inter-lots ---------------------------------
...
declare
  ...
  n_pat_upd    int := 0;
  n_enc        int := 0;
  n_already    int := 0;  -- §7.8 : lignes deja importees par un lot anterieur (ignorees)
  errors       jsonb := '[]'::jsonb;
  seen_codes   text[] := array[]::text[];
  seen_rows    text[] := array[]::text[];
  ok_rows      text[] := array[]::text[]; -- §7.8 : empreintes des lignes REUSSIES (flush autonome)
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_rows, '[]'::jsonb)) loop
    idx := idx + 1;
    begin
      v_rowkey := null; -- reinitialise a CHAQUE ligne (une ligne sans rencontre n'a pas d'empreinte)
      ...
      -- §6.3 doublon INTRA-fichier + §7.8 idempotence INTER-lots.
      if v_enc is not null and jsonb_typeof(v_enc) = 'object' then
        v_rowkey := v_code || '|' || coalesce(v_enc ->> 'encounter_date', '') || '|'
                    || coalesce(v_enc ->> 'encounter_type', '') || '|' || md5(coalesce(v_enc -> 'data', '{}'::jsonb)::text);
        -- §7.8 : ligne DEJA importee avec succes par un AUTRE lot de cette base -> IGNOREE
        -- (ni re-creee, ni comptee en erreur). Rejouer un fichier corrige ne duplique donc
        -- plus les lignes qui avaient reussi. (Vaut aussi pour l'APERCU : le medecin voit
        -- « deja importees » AVANT de committer.)
        if exists (
      ...
      -- §7.8 : empreinte conservee UNIQUEMENT pour une ligne totalement reussie (une ligne en
      -- erreur voit son sous-bloc annule cote lot ; en autonome, ok_rows n'est rempli qu'ici).
      if v_rowkey is not null then ok_rows := ok_rows || v_rowkey; end if;
  ...
  if not p_dry_run then
    if p_batch_id is not null then
      ...
      -- §7.8 : l'import AUTONOME laisse lui aussi ses empreintes (lignes reussies seulement)
      -- -> il participe a l'idempotence inter-lots comme les imports par chunks.
