...
const formatValue = (v: unknown, type?: string): unknown => {
  ...
  // D14 : conserver les nombres natifs JS.
...
export function mergeExportFields(input: ExportField[]): ExportField[] {
  ...
  for (const field of sorted) {
    ...
    if (previous) {
      previous.templateVersionIds = [
        ...new Set([...(previous.templateVersionIds ?? []), ...versions]),
      ].sort();
      ...
      previous.allowedOptions = mergeOptionLists(
        previous.allowedOptions,
