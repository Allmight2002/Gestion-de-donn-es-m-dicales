// Handler testable de cleanup-upload (audit lot 9 §C3) : suppression serveur d'un objet Storage
// orphelin apres echec d'insertion metier, sous verification RLS + propriete du ticket. Effets externes
// (clients Supabase, horloge) injectes ; index.ts est l'adaptateur Deno.serve.
import type { SupabaseClient } from '@supabase/supabase-js';
import { readJsonObject, validationResponse } from '../_shared/contracts.ts';
...
const BUCKETS = {
  'clinical-attachments': { authRpc: 'can_write_identity', table: 'clinical_attachment', column: 'storage_path' },
  'raw-documents': { authRpc: 'is_base_owner', table: 'raw_document', column: 'storage_path' },
  'scientific-exports': { authRpc: 'can_export_data', table: 'export_log', column: 'stored_file_path' },
} as const;
...
export async function handleCleanupUpload(req: Request, deps: CleanupDeps): Promise<Response> {
  ...
  if (!bucketConfig || !baseId || typeof payload.path !== 'string' || !payload.path.startsWith(`${baseId}/`)) {
  ...
  if (Date.parse(ticket.expires_at) <= deps.now()) {
    await admin.from('upload_ticket').update({ status: 'expired', last_error: 'expired before cleanup' }).eq(
      'id',
      ticket.id,
    );
  ...
  if (removeErr) {
    await admin
      ...
      .eq('id', ticket.id)
      .eq('status', 'cleaning');
    return json(500, { error: 'Suppression Storage impossible' });
  }

  await admin
    .from('upload_ticket')
    .update({ status: 'cleaned', cleaned_at: deps.nowIso(), last_error: null })
    .eq('id', ticket.id)
    .eq('status', 'cleaning');
